...
export function ClassDetailPage({ classId }: { classId: string }) {
  ...
  return (
    <>
      ...
      <div className="tabs"><button className={`tab-btn ${tab==='overview'?'active':''}`} onClick={()=>setTab('overview')}>Overview</button><button className={`tab-btn ${tab==='schedules'?'active':''}`} onClick={()=>setTab('schedules')}>Schedules</button><button className={`tab-btn ${tab==='sessions'?'active':''}`} onClick={()=>setTab('sessions')}>Sessions</button><button className="tab-btn">Students (Phase 5)</button><button className={`tab-btn ${tab==='attendance'?'active':''}`} onClick={()=>setTab('attendance')}>Attendance</button></div>
      ...
      {tab==='attendance' ? <div><button className="ms-button" onClick={()=>navigateTo(`/classes/${item.id}/attendance-summary`)}>View attendance summary</button></div> : null}
    </>
  )
}
